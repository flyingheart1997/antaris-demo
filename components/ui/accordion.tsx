...
>(({ className, ...props }, ref) => (
  <AccordionPrimitive.Item
    ref={ref}
    className={cn("border-b border-stroke-primary", className)}
    {...props}
  />
))
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between py-4 text-sm font-medium transition-all hover:text-text-selected text-left [&[data-state=open]>svg]:rotate-180 text-text-primary",
        className
      )}
      {...props}
    >
